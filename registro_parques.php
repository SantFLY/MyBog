...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Get form data
    $nombre_parque = $_POST['nombre_parque'];
    $ubicacion_parque = $_POST['ubicacion_parque'];
    $tipos_de_parques = $_POST['tipos_parque'];
    $informacion_parque = $_POST['informacion_parque'];
    $localidad = $_POST['localidad'];
    $id_entretenimiento = $_POST['id_entretenimiento'];

    $archivos = $_FILES['photos']['name'];
    $carpeta_destino = 'Imagenes/parques/';
    if(!empty($archivos)) {
        ...
        foreach($archivos as $key => $archivo) {
            ...
            $nombre_archivo = $nombre_parque.'.'.$extension;
    ...
    $sql = "INSERT INTO parques (Nombre_de_parques, Ubicacion_de_parques, Tipos_de_parques, Informacion_de_parques, Id_entreteniemiento, localidad, nombre_imagen) 
        VALUES (?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conexion->prepare($sql);

    if($stmt) {
        $stmt->bind_param("ssisiss", $nombre_parque, $ubicacion_parque, $tipos_de_parques, $informacion_parque, $id_entretenimiento, $localidad, $nombre_archivo);
...
<body>
    <div class="wrapper">
        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <div class="login-box">
                        <form action="registro_parques.php" method="post" enctype="multipart/form-data">
                            <div class="form-group" id="nombre_parque">
                                <label for="nombre_parque">Nombre del Parque</label>
                                <input type="text" class="form-control" name="nombre_parque" maxlength="250"
                                    pattern="[^;]*"
                                    title="Ingresa el nombre del parque (máximo 250 caracteres, no se permite el caracter ';')"
                                    required>
                            </div>

                            <div class="form-group" id="ubicacion_parque">
                                <label for="ubicacion_parque">Ubicación</label>
                                <input type="text" class="form-control" name="ubicacion_parque" maxlength="250"
                                    pattern="[^;]*"
                                    title="Ubicación (máximo 250 caracteres, no se permite el caracter ';')">
                            </div>

                            <div class="form-group" id="tipos_parque">
                                <label for="tipos_parque">Tipos de Parque</label>
                                <select class="form-control" id="tipos_parque" name="tipos_parque"
                                    title="Selecciona el tipo de parque" required>
                                    <option value="" disabled selected></option>
                                    <option value="1">Mecanicos</option>
                                    <option value="2">Acuaticos</option>
                                    <option value="3">Infantil</option>
                                    <option value="4">Bebes</option>
                                    <option value="5">Urbano</option>
                                    <option value="6">Tematico</option>
                                </select>
                            </div>

                            <div class="form-group" id="informacion_parque">
                                <label for="informacion_parque">Información del Parque</label>
                                <textarea type="text" class="form-control" id="informacion_parque"
                                    name="informacion_parque" maxlength="250" pattern="[^;]*"
                                    title="Ingresa la descripción que quieres darle a tu parque (máximo 250 caracteres, no se permite el caracter ';')"
                                    required></textarea>
                            </div>
                            ...
                            <div class="form-group" id="localidad">
                                ...
                                <select class="form-control" id="localidad" name="localidad"
                                    title="Selecciona la localidad en la que se ubica tu parque" required>
                                    <option value="" disabled selected></option>
                            ...
                            <div class="input-group mb-3">
                                ...
                                <div id="image-preview" class="image-preview"
                                    title="Selecciona las imágenes de tu parque que quieres mostrar."></div>
                            </div>
    ...
    <div class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="3000" style="display:none">
        ...
        <div class="toast-body">
            <?php
            echo "El parque ha sido registrado correctamente";
            ?>
        </div>
    </div>
